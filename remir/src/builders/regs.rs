...
pub fn build_copy(module: &mut Module, val: BaseSSAValue) -> Result<BaseSSAValue, ()> {
    let inst = Instruction::Copy { val };

    module.write(inst).get()
}

pub fn build_load(module: &mut Module, source: SSAPointerValue) -> Result<BaseSSAValue, ()> {
